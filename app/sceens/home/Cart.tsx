  ...
  return (
    <View style={{ flex: 1 }}>
      <ScreenWrapperNoScroll>
        ...
        {
          productState.cart.items.length === 0 ? (<View style={{ flex: 1, justifyContent: 'center', alignItems: 'center' }}>
            <Label style={{ fontWeight: 400 }}>No items in cart</Label>
          </View>) : (
            <FlatList
              data={cartItems}
              ListFooterComponent={() => {
                const [promoCode, setPromoCode] = useState(productState.promoCode);
